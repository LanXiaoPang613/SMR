...
parser.add_argument('--data_path', default='C:/Users/pc/Desktop/dataset/cloth1m/images', type=str, help='path to dataset')
...
def warmup(net,optimizer,dataloader):
    ...
    for batch_idx, (inputs, labels, path) in enumerate(dataloader):      
        ...
        sys.stdout.write('|Warm-up: Iter[%3d/%3d]\t CE-loss: %.4f  Conf-Penalty: %.4f'
                %(batch_idx+1, args.num_batches, loss.item(), 0.))
        sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    for epoch in range(args.num_epochs+1):
        ...
        elif (epoch + 1) % mid_wp == 0 and epoch >= 1:
            warmup_trainloader = eval_loader#loader.run('warmup')
            print('Warmup Net1')
        ...
        if best1 <= acc:
            best1 = acc
        log.write('Testing Epoch:%d      Acc:%.2f  best acc:%.2f\n' % (epoch, acc, best1))
        log.flush()
        print('Tesing Epoch:%d      Acc:%.2f  best acc:%.2f\n' % (epoch, acc, best1))
    ...
    log.write('Val Test Accuracy:%.2f\n'%(acc))
    log.flush()
    print('Val Test Accuracy:%.2f\n' % (acc))
